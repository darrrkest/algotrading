...
import com.example.connector.ConnectionStatus;
import com.example.messages.QLEnvelope;
import com.example.messages.QLEnvelopeAcknowledgment;
...
public final class QLAdapterImpl implements QLAdapter {
    ...
    private ConnectionStatus _connectionStatus = ConnectionStatus.UNDEFINED;
    ...
    public QLAdapterImpl(String ipAddress, int port) {
        ...
        this.port = port == 0 ? QLConnector.DEFAULT_PORT : port;
    }
    ...
    public void start() {
        synchronized (lock) {
            if (ConnectionStatus.isActive(_connectionStatus)) return;
    ...
    private void runConnect() throws InterruptedException {
        Thread.currentThread().setName("QL_CONN");
    ...
    private void runReceive() throws InterruptedException {
        Thread.currentThread().setName("QL_RECV");
    ...
    private void runDeserialize() {
        Thread.currentThread().setName("QL_DESER");
