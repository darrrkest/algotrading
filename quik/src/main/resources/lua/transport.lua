...
transport.outputQueue = {} -- очередь исходящих сообщений
local sentCount = 0
...
messageTypes.StopOrderStateChange = "StopOrderStateChange"
...
messageTypes.InstrumentParamsSubscriptionRequest =
    "InstrumentParamsSubscriptionRequest"
messageTypes.InstrumentParamsUnsubscriptionRequest =
    "InstrumentParamsUnsubscriptionRequest"
messageTypes.TicksSubscriptionRequest = "TicksSubscriptionRequest"
messageTypes.TicksUnsubscriptionRequest = "TicksUnsubscriptionRequest"

-- положить сообщение в очередь на отправку
function transport:putMessage(message)
    if transport.outputQueue == nil then return end

    table.sinsert(transport.outputQueue, message)    
end
...
local function popMessage()
    if #transport.outputQueue == 0 then return nil end
    return table.sremove(transport.outputQueue, 1)
end

-- очистить очередь исходящих сообщений
local function clearOutputQueue()
    while #transport.outputQueue > 0 do
        table.sremove(transport.outputQueue, 1)
    end
end
...
local function createServer()
    ...
    sentCount = 0
    server = socket.bind('127.0.0.1', config.transport.port, 0)
	--server = socket.bind('127.0.0.1', 15946, 0)
    server.settimeout(server, 1)
end

-- инициализация транспорта
function transport:init(clientConnectedCallback, clientDisconnectedCallback,
...
function transport:MakeEnvelope()
    ...
    if #transport.outputQueue > 0 then
        trace('debug', 'Messages in queue: ' .. #transport.outputQueue)
    end
...
function transport:cycle()
    ...
    if not clientConnected then
        ...
        if client == nil then
            if waitingForClientLogged == false then
            ...
            trace("info", "Client connected, Sent count = " .. sentCount ..
                      ", LastEnvReceivedByClient = " .. lastEnvReceivedByClient)
            if clientConnectedHandler then clientConnectedHandler() end
        end
    else
        ...
        if lastEnvReceivedByClient == sentCount then
            ...
            if envelope.count > 0 then
                ...
                if envelopeJson ~= nil then
                    ...
                    while sentSize < envelopeSize do
                        ...
                        local ok, count, err, count2 =
                            pcall(client.send, client, envelopeJson,
                                  sentSize + 1, envelopeSize)

                        if err ~= nil then
                            trace("error",
                                  "Error while pcall(client.send...): " .. err)
                            count = count2

                            -- если отправка не прошла по таймауту, то немного засыпаем, чтобы клиент прочухался
                            if err == "timeout" then
                                trace("warn",
                                      "Sleep 100ms after socket send timeout")
                                threadSleep(100)
                        ...
                        sentSize = count
                        trace('debug',
                              'Sending envelope ' .. sentCount .. '. Sent ' ..
                                  sentSize .. ' of ' .. envelopeSize)
                    end

                    -- ошибка сериализации конверта
                else
                    trace('error', 'Envelope wasnt serialized: ' ..
                              envelope.count .. " messages in envelope.")
                end
            ...
            if waitingForAckLogged == false then
                trace('debug',
                      "Waiting for ack " .. sentCount ..
                          " from client. Last ack received is " ..
                          lastEnvReceivedByClient)
        ...
        if status and requestString then
            ...
            if not ok then
                trace("error", clientMessage);
                return
            end

            if clientMessage.message_type == nil then
                trace("error",
                      "Client message doesn't have a 'message_type' property")
                return
            end
