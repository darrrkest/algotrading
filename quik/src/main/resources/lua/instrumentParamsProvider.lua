...
function ip:Setup(settings)
  self.receiveMarketdata = settings.receiveMarketdata 
end
...
function ip:subscribe(request)  
  ...
  local classCode = codes:getClassCode(request.instrument)
...
function ip:processParams( class, sec )
  ...
  if self.receiveMarketdata == false then
    return
  end
  ...
  if class =="SPBFUT" or class == "SPBOPT" or class == "OPTW" then 
    local p={}
    ...
    p.openinterest = tonumber(par.param_value)
    par = getParamEx(class, sec, "EXPDATE")
    p.expiredate = tonumber(par.param_value)
	  --p.BIGFIELD = ""
...
function ip:clearSubscriptions()
  trace("info", "Unsubscribe client from all instruments")
  self.subscriptions = {}
end
